...
# Optional: install Playwright only for connectors or manual checks that need browser rendering
...
| Stage | What it does |
|-------|-------------|
| **discover** | Fetches the staff directory landing page, extracts result page links and department links, and stores connector metadata used to build request-aware result links |
| **crawl** | Follows result page links with pagination. Result links may be HTML GETs or JSON POST requests. Linked personal pages, CVs (PDF), and CRIS pages are also fetched. All content is stored as content-addressed artifacts in `data/raw/` |
| **parse** | Reads stored HTML and JSON result artifacts, routes through the appropriate parser (results page vs personal page), and merges records by `person_id`. PDFs are text-extracted and attached as artifacts |
...
| **export** | Writes a timestamped snapshot like `data/exports/people_YYMMDD_HHMMSS.json` or `.jsonl` |
...
data/
  raw/
    ...
    json/          # Downloaded JSON result pages
  ...
  exports/         # timestamped snapshots like people_260414_123456.json
  state/
    discovery.json       # Last discovery snapshot
    fingerprints.json    # Request key -> SHA256 checksum map for incremental reruns
    crawl_manifest.json  # Request-aware crawl manifest for the current run
```

`data/`, local `ou_harvest.toml`, and other scratch outputs are intended to stay local and are gitignored. Do not commit generated pipeline outputs such as `data/exports/`, `data/records/`, or `data/raw/` to a public repository.
...
The shared `ou_harvest.toml` never contains the actual token, and the local runtime `ou_harvest.toml` is kept out of git.
...
| `playwright` | playwright>=1.44 | Browser-rendered page fetching when a connector or manual check needs it |
...
Tests use a mix of small checked-in synthetic fixtures and larger real-site fixtures generated on demand. To fetch the real-site fixtures locally, run:

```bash
python3 scripts/fetch_test_fixtures.py
```

That script writes local-only fixtures such as `bgu_page_data.json`, `bgu_search_page_1.json`, selected BGU profile pages, and the Technion fixtures. If those generated fixtures are absent, the dependent tests skip cleanly. The `pythonpath` is configured to `src` in `pyproject.toml`. Some tests use `unittest.mock` and `monkeypatch` for stubbing HTTP responses and pipeline factories.
